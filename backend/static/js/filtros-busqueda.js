...
function exportarPDF(){
  ...
  const css = `
    <style>
      *{box-sizing:border-box}
      ...
      .p-row{display:grid;grid-template-columns:28px 1fr auto;gap:12px;align-items:center;
  ...
  const rowsHTML = lastResults.map(it => `
    <div class="p-row">
      <div class="p-ico">${iconoRolPrint(it.rol)}</div>
      <div>
        <div class="p-main">
          ${escapeHTML(it.usuario)} — ${escapeHTML(mapAccion({accion: it.accion, exito: it.exito}))}
        </div>
        <div class="p-sub">
          ${escapeHTML(formatFecha(it.fechaHora))} · ${escapeHTML(it.nombreCompleto)} ·
          <span class="badge">${escapeHTML(it.estado)}</span>
        </div>
      </div>
      <div class="rlabel">${it.rol==='admin'?'Administrador':'Secretaría'}</div>
    </div>
  `).join('');

  const html = `<h2>Resultados — Filtro de Búsqueda</h2><div class="list">${rowsHTML}</div>`;

  win.document.write(`<html><head><title>Exportar búsqueda</title>${css}</head><body>${html}</body></html>`);
  // pequeño delay para que el motor de render pinte antes de imprimir
  setTimeout(() => { try { win.document.close(); win.focus(); win.print(); } catch(_){} }, 50);
}
...
function mapAccion(l){
  ...
  const map = {
    ...
    self_edit:     'Edición de su cuenta',      // alias antiguo
    user_edit_self:'Edición de su cuenta',      // alias nuevo
    report:        'Generación de reportes',    // alias antiguo
    report_gen:    'Generación de reportes',    // alias nuevo
