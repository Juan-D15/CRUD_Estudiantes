// Login JS para admin. Maneja: validaciones de campos, compatibilidad con reCAPTCHA v2,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function validateLogin() {
    ...
    // 1) Si existe captcha simple en el DOM, lo validamos.
    if (capQ && capA) {
      const val = capA.value.trim();
      if (val === '' || Number(val) !== capAns) { text(eC, 'Captcha incorrecto.'); ok = false; }
    } else {
      // 2) Si hay Google reCAPTCHA, validar que esté marcado (getResponse no vacío)
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      // Si pasa validación, NO prevenimos: dejamos que Django procese el POST
    });
  }
  ...
  if (btnRecover) {
    btnRecover.addEventListener('click', () => {
      ...
      window.location.href = 'admin-reset.html';
    });
  }
});
